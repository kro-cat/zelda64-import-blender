...
class MapLoader:
    ...
    def load_map(self, address: int):
        pass

    def load_maps(self, strategy: MapLoaderStrategy):
        match strategy:
            case MapLoaderStrategy.NO_DETECTION:
                self.import_map_with_headers()
            case MapLoaderStrategy.BRUTEFORCE:
                self.search_and_import(3, False)
            case MapLoaderStrategy.SMART:
                self.import_map_with_headers()
                self.search_and_import(3, True)
            case MapLoaderStrategy.TRY_EVERYTHING:
                self.import_map_with_headers()
                self.search_and_import(3, False)

    def import_map_with_headers(self):
